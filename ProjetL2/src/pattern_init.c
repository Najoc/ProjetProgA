...
Attaque* tir_soldat_2(SDL_Renderer* renderer){
    ...
    for(int i = 0; i < 50; i++){
        tir[i].x = 1 + (2*(i/10)); tir[i].y = i%10; tir[i].degats = 10;
    }
...
Attaque* tir_soldat_6(SDL_Renderer* renderer){
    Tile* tir = malloc(sizeof(Tile)*50);
    for(int i = 0; i < 50; i++){
        tir[i].x = i%10; tir[i].y = (2*(i/10)); tir[i].degats = 10;
    }
...
Attaque* proximiteNS(SDL_Renderer* renderer){
    ...
    for(int i = 0; i < 7; i++){
	for(int j = 0; j < i+1; j++){
	    mine[k].x = 0 + i - j; mine[k].y = 0 + j; mine[k].degats = 100 - (10*i);
	    k++;
	}
    }
    for(int i = 0; i < 7; i++){
	for(int j = 0; j < i+1; j++){
	    mine[k].x = 9 - i + j; mine[k].y = 9 - j; mine[k].degats = 100 - (10*i);
	    k++;
	}
    }
...
Attaque* proximiteEO(SDL_Renderer* renderer){
    ...
    for(int i = 0; i<7; i++){
        for(int j = 0; j < i+1; j++){
	    mine[k].x = 9 - i + j; mine[k].y = 0 + j; mine[k].degats = 100 - (10*i);
	    k++;
	}
    }
    for(int i = 0; i<7; i++){
        for(int j = 0; j < i+1; j++){
	    mine[k].x = 0 + j; mine[k].y = 9 - i + j; mine[k].degats = 100 - (10*i);
	    k++;
	}
    }
...
Attaque* laser(SDL_Renderer* renderer){
    ...
    for(int i = 0; i < 40; i++){
	laser[i].x = 3 + (i/10); laser[i].y = i%10; laser[i].degats = 25;
    }
...
Attaque* laser2(SDL_Renderer* renderer){
    Tile* laser = malloc(sizeof(Tile)*40);
    for(int i = 0; i < 40; i++){
	laser[i].x = i%10; laser[i].y = 3 + (i/10); laser[i].degats = 25;
    }
    Attaque* tirLaser = initialiser_attaque("images/attaqueTest.bmp", renderer, "laser", 'e', 0, laser, 40, TILE_WIDTH, TILE_HEIGHT);
    return tirLaser;
}
